...
class CCodeGenerator:
    ...
    def _walk_nodes(self, node: int):
        ...
        # ── FOR INIT block ───────────────────────────
            ...
            # Find FOR NEXT — search body AND body's successors
            if body_node is not None:
                # Search direct successors of body for FOR NEXT node
                body_succs = list(
                    self.G.successors(body_node)
                )
                # Also check body itself (some CFG builders put it inline)
                all_candidates = [body_node] + body_succs
                for bs in all_candidates:
                    ...
                    if "FOR NEXT:" in bs_label:
                        ...
                        # Strip the "FOR NEXT:" prefix properly
                        inc = re.sub(
                            r'^FOR NEXT:\s*', '', inc_line
                        ).strip()
